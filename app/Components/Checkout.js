...
export default function Checkout({ initial }) {
  const router = useRouter();
  const {
    title = "No product",
    ...
    <div className="max-w-6xl p-2 mx-auto mt-24 mb-6">
      ...
      <div className="p-4 mb-6 border rounded-lg bg-gray-50">
        ...
        <div className="flex items-center justify-between">
          ...
          <p className="font-bold text-green-600">₹{Number(price).toLocaleString()}</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="p-4 space-y-4 bg-white border rounded-lg">
        <div>
          <h3 className="mb-2 text-lg font-semibold">Contact</h3>
          <input type="email" name="email" value={form.email} onChange={handleChange} placeholder="Email or Mobile Number" className="w-full p-2 border rounded" required />
        </div>
        ...
        <div>
          ...
          <input name="address" value={form.address} onChange={handleChange} placeholder="Address" className="w-full p-2 mt-3 border rounded" required />
          <div className="grid grid-cols-1 gap-3 mt-3 md:grid-cols-3">
            <input name="city" value={form.city} onChange={handleChange} placeholder="City" className="p-2 border rounded" required />
            <input name="state" value={form.state} onChange={handleChange} placeholder="State" className="p-2 border rounded" required />
            <input name="pincode" value={form.pincode} onChange={handleChange} placeholder="PIN Code" className="p-2 border rounded" required />
          </div>
          <input name="phone" value={form.phone} onChange={handleChange} placeholder="Phone" required className="w-full p-2 mt-3 border rounded" />
        </div>
